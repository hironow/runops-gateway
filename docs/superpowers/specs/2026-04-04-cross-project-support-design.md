...
### IAM 前提条件: `artifactregistry.reader`

Cloud Run はトラフィック切り替え時に新しいリビジョンのコンテナイメージを pull する。イメージが APP_PROJECT の Artifact Registry にある場合、chatops SA に `roles/artifactregistry.reader` が必要。この権限がないと ShiftTraffic がイメージ pull 失敗で PermissionDenied になる。
