...
```bash
just dev          # 推奨: .env.local (git 管理の local-dev env) を source して起動
# または手動 env で:
just run
go run ./cmd/server
```

`just dev` は git 管理された `.env.local` (→ 任意の `.env`) を source する。`.env.local` は
dotfiles の共有スタックを前提に `PUBSUB_EMULATOR_HOST=localhost:9399` /
`FIRESTORE_EMULATOR_HOST=localhost:8080` / `OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4317`
等を持つ (local/dummy 値のみ、実 secret は禁止)。dotfiles checkout 側で
`just emu-up-only firebase-emulator` + `just tel-up` を起動しておくと、Pub/Sub / Firestore /
trace がすべて繋がる。`just run` は env を自前で export したいとき用。
