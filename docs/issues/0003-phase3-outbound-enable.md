...
**Status:** 📝 未着手 (Issue 0001 Phase 3 完了待ち)
**Depends on:** Issue 0001 Phase 3 (= dmail-emitter が production workspace VM 上で稼働し、 `dmail-outbound` topic に publish が始まること)。 Issue 0001 自体は 2026-05-06 時点で Phase 1 (placement + tests) GREEN、 Phase 2 (image build + AR publish) は本リポの local commit 済 push 待ち、 Phase 3 (dotfiles 側 image variable 切替 + workspace VM 再起動) は production 操作待ち
