...
+---------+---------+---------------------------+
          |         |
          |         +------ roles/run.developer (project-level) -----+
          |         |                                                |
          |         v                                                v
+---------+--------------------+    +-------------------------------+--------+
...
- roles/run.developer: Cloud Run の操作権限（各 APP_PROJECT のプロジェクトレベルで付与）
...
| リスク | 対策 |
|---|---|
| chatops SA の権限が広がりすぎる | `run.developer` はプロジェクトレベルで付与する（LRO ポーリングに必要）。`iam.serviceAccountUser` はランタイム SA 単位で付与する。APP_PROJECT に管理対象アプリのみが存在することを確認する |
...
- **`run.developer`**: プロジェクトレベルで付与する。1 つのバインディングでプロジェクト内の全 Cloud Run リソースを操作できるため、アプリが増えても追加不要。
...
# run.developer はプロジェクトレベルで 1 回だけ付与すれば全 Cloud Run リソースを操作できる
# 1 アプリ目のセットアップ時に実行済みであれば追加不要
gcloud projects add-iam-policy-binding SHARED_APP_PROJECT \
  --member="serviceAccount:${CHATOPS_SA}" \
  --role="roles/run.developer"
