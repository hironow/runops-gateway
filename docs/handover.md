...
| **Phase 1 (新)** | 🟡 完了 + 後追い修正中 (2026-05-05) | **シンプル経路**: `/slack/command` で Slash Command 受信 → 直接 dispatch → thread reply (StubDispatcher は slog のみ)。確認ボタン step は Phase 4 に移送。Codex Review で発見された replay protection を Issue 0019 で同 PR 内追加 |
...
### F-4: StubDispatcher の slog 出力に dispatch text が含まれる (要監視)

```go
// internal/adapter/output/dispatcher/stub.go
d.logger.LogAttrs(ctx, slog.LevelInfo, "dispatched stub",
    slog.String("role", string(req.Role)),
    slog.String("text", req.Text),  // ← 機密情報が混入する可能性
    slog.String("requester_id", req.RequesterID),
    ...
)
```

production で `/agent paintress fix M-42 (token=AKIA...)` のような形で
**機密情報を text に含めて投げられた場合、Cloud Logging に流れる**。

**致命度**: 中。Phase 1 は社内 (個人) 運用前提なので即座のリスクは低いが:

- Phase 4 (本番化) までに **dispatch text のサイズ・サニタイズポリシー** を策定する必要
- 当面は intent.md の「PII / secret は emit しない」ルール (observability-standards) に
  従い、operator 教育で対応

**追跡先**: Issue として未起票。Phase 4 着手時に Issue 0020 として起票予定。

### F-5: Phase 1 で Block Kit 確認ボタンを Phase 4 に移送 (Issue 0018 と差分あり)

Issue 0018 の TDD 計画は「Slash Command → Block Kit 確認 → 承認時に dispatch」の
2 ステップだったが、実装は **1 ステップ (Slash Command が直接 dispatch)** に
簡略化した。理由は Issue 0018 冒頭の「実装後の差分メモ」に記載。

**致命度**: 低。Phase 4 で HIGH severity approval gate を実装する際に
確認ステップを挿入する形で対応する。`TestInteractiveHandler_DispatchActionIDsDoNotRouteToApprove`
で Phase 4 の dispatch_* action_id 衝突を予防済み。
