...
### Hardening (2026-05-08 後半 phase 3、 framework + gate codification)

session 終盤で codex (gpt-5.5) retrospective 提案 4 件のうち 2 件を着地:

| PR | 内容 |
|---|---|
| #102 | **gateway justfile fmt-check gate**: \`gofmt -l .\` で drift 検出する read-only recipe + \`check\` recipe を \`fmt → fmt-check\` に置換。 PR #94 fixup 自爆 pattern (= drift PR で canonical pin したばかりで Metadata map alignment を手書きして CI fail) の再発防止 structural gate |
| (CLAUDE.md edit、 git 管理外) | **\`/Users/nino/.claude/CLAUDE.md\` ADR template §Enforcement inventory 必須化**: architectural-pin / security-invariant ADR の起案時に「Entry points / Persistent data / Bypass candidates / Tests」 4 セクションを必須化。 ADR 0035 が Phase 4a path を見落とした root cause (= enforcement point inventory 不在) を framework レベルで防止する long-term lever |

4 tool (phonewave / sightjack / paintress / amadeus) は既に \`lint:\` recipe 内で \`gofmt -l . | grep . && exit 1\` 同等の drift 検出 gate を実装済のため、 fmt-check の cross-tool 展開は不要 (= consistency 観点で gateway の単独 \`fmt-check\` recipe vs 4 tool の lint embed の API surface 違いはあるが、 効果は同等)。

### 残 work (次 session 候補)

- **producer-side per-tool ADR + implementation**: phonewave / sightjack / paintress / amadeus 各 repo で per-tool ADR 起案 + DMail emit に \`RUNOPS_ACTOR_TYPE\` env reading + metadata 設定 + \`actor_type_source=env\` + \`initiating_actor_type\` (daemon 用 systemd unit env)。 各 tool での actor type 判断 logic decide (= AI agent driving vs human CLI vs scheduled workspace daemon の区別) を ADR で固める必要、 cross-repo 4 PR sequence
- **ADR 0038 candidate**: dispatch-path fail-closed flip + broker-only \`ai-agent\` (rollout coverage 確認後)
- **fmt-check API surface 統一** (任意): 4 tool に gateway PR #102 同 pattern (= 単独 \`fmt-check\` recipe) を展開すれば \`just fmt-check\` 単独呼び出しが cross-tool で可能になる。 効果は documentation 化のみで重複実装回避ベース、 priority 低
