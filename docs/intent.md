...
+------------------+                  |                                             |
                                      ...
                                      |  d. Send Block Kit UI (with JSON payload)   |
                                      +---------------------------------------------+
                                                            |
                                                            | POST Webhook
                                                            v
              ...
              |  - V1: 90% (Previous)      |   | 1. On-Demand Backup    |
              |  - V2: 10% (Canary)        |   +------------------------+
              +----------------------------+              |
                                                         v
                                               +------------------------+
...
- Cloud Run Service: HTTPトラフィックを処理するサービス
...
- Cloud Build はビルド・デプロイ（トラフィック 0%）と Slack 通知のみを担当
...
```
# 承認操作
runops approve service frontend-service --action=canary_10
