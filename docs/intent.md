...
> **Draft notice (2026-05-05)**: 本セクションは **Phase 2 以降の意図**である。
> 当初は Phase 1 から D-Mail / Pub/Sub bridge を実装する想定で書かれたが、
> 粒度が大きいことから方針転換し、**Phase 1 は「シンプル経路」（Slack 内で完結する
> stub dispatch、D-Mail なし）** に差し替えた。Phase 1 の意図は
> [`docs/issues/0018-simple-message-path.md`](issues/0018-simple-message-path.md)
> に分離し、関連 ADR (0012/0013/0015) は **Proposed (draft)** ステータスのまま
> Phase 2 着手時に再評価する。
>
> 本セクション全体は **将来の方向性として残す** が、Phase 1 完了までは確定事項では
> ないことに注意。設計の輪郭・3 つの判断 (A/B/C) はそのまま参考になる。

> このセクションは Phase 0（既存 ChatOps）の上に積み増す **Phase 2 以降の拡張意図**
