...
- **workspace VM(s)**: per-workspace で provisioned (`coder-${owner}-${ws_name}-root`)、 debian-12、 e2-small、 `tag:exe-workspace`、 startup-script で tailscale + docker + `docker run --restart=unless-stopped` で devcontainer image を起動 (devcontainer は long-lived、 dmail container とは異なる lifecycle)
- **template**:
...
- `exe/coder/templates/dotfiles-devcontainer/main.tf` の startup_script は既に `docker run --restart=unless-stopped` で devcontainer を起動する流儀 (line 308-319、 ただし dmail container 側は `--rm` のみ + systemd `Restart=on-failure` で supervision する。 `--rm` と `--restart` は docker engine が同時指定を拒否する mutually exclusive オプション)
...
Issue 0001 の **配置先 = workspace VM の host OS systemd**、 **起動方式 = `docker run --rm` + systemd `Restart=on-failure`** で確定。 `--rm` と `--restart=unless-stopped` は docker engine が同時指定を拒否する (mutually exclusive、 [docker run docs](https://docs.docker.com/reference/cli/docker/container/run/)) ので、 supervision は systemd 側に寄せる。 supervisor 系 (supervisord / s6-overlay / systemd-user) は使わない。
...
2. **dotfiles 側 systemd unit**:
   ...
   - `docker run --rm` で daemon container 起動 (再起動は systemd `Restart=on-failure` が担当)
