...
| dmail-receiver / dmail-emitter | **未 deploy** (別リポ `hironow/dotfiles` で systemd 化予定)。本リポ側は backlog-stale alert で監視 |
...
# gh variable set EXE_CODER_VM_SA_EMAIL        --body "dmail-bridge@${PROJECT}.iam.gserviceaccount.com" --repo "${REPO}"
...
```
                                                    [ exe-coder VM ]
                                                      systemd:
cmd/server                              cmd/runops      - dmail-receiver
    |  HTTP                                 |  CLI      - dmail-emitter
    |  /slack/{command,interactive}         |
    +-------- [ UseCase ] -------+----------+              ^
                  |                                        |
...
│   ├── dmail-receiver/     # exe-coder VM systemd: Pub/Sub -> phonewave outbox
│   └── dmail-emitter/      # exe-coder VM systemd: 5 本柱 archive watch -> Pub/Sub
├── internal/
│   ├── core/
